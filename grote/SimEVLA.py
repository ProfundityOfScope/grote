...
class MotionSimulator(object):
    """
    A motion simulator for the EVLA.
    ...
    def __init__(self, startaz=None, startel=None, wrap=None):
        ...
        Create a EvlaTelescopeMotionSimulator with an initial position of
        ...
        if w != 'CLOCKWISE':
            ...
            if goOverTheTop:
                ...
                if (tmpAz >= self.__MIN_AZ and tmpEl <= self.__MAX_EL):
                    tmin, updateBool = self.__updateMin(tmin,
                                                        tmpAzEnd,
                                                        tmpElEnd)
                    if updateBool:
    ...
    def getCurrentAntennaWrap(self):
        ...
        if self.__currentAz > 180*u.deg:
            return 'CLOCKWISE'  # Was: AntennaWrap.CLOCKWISE
        else:
            return 'COUNTERCLOCKWISE'  # Was: AntennaWrap.COUNTERCLOCKWISE

    def getCurrentAntennaAzimuth(self):
        """
        Return current antenna azimuth.
...
def EquitorialToHorizontal(ra, dec, lst):
    """
    Convert equitorial coordinates to local at a given LST.
